...
앞서 <Lab: 리스트의 내용을 파일로 저장>저장에서 만든 **pitcher_stats.txt** 파일로부터 투수 통계 데이터를 읽어와 아래와 같이 텍스트 데이터를 파싱한다. 
...
<Lab: 투수 통계 데이터 파싱>과 같은 방식으로 **pitcher_stats.txt** 파일을 파싱해서 읽어들인 후 **pitcher_stats_new.txt**라는 새로운 파일 이름으로 저장하는 프로그램을 구현해보자. 
