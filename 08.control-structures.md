...
정수 **1**부터 **10**까지 포함하고 있는 뜌플(또는 리스트)를 생성한 후 변수 ***integers***에 할당한다.

**while**문으로 뜌플(또는 리스트)를 순환해서 홀수만 출력한다.
- 홀수는 **x % 2**로 구할 수 있다.
...
**힌트**   
- `print()` 함수의 매개변수 ***end***의 값을 공백 하나(**' '**)로 설정하면 새줄바꿈 대신 한칸 비움으로 출력이 가능하다.
- 예) `print(n, end=' ')`
